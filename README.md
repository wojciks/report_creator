...
+ Navigation calculations: total voyage time, currents, LT, voyage distances and average speeds (this needs to be improved).
...
+ Noon reports for days with more than one event (e. g. Noon > BOSP > Arrival > Noon, so the last noon report will take into account fuel consumption not only from last event, but from last Noon report)
+ Voyage 
...
+ Multiline field "Remarks:" in `Nav info` tab should update itself according to last database entry. The problem is the new line character `\n` which is literally put in the field. For now, this field will not update from database.
+ Latitude / longitude hemispheres as a choice in dropdown list does not allow to get a default value from database's last entry.